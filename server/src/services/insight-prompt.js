...
export function userPrompt(role, appliedFilters) {
  ...
  const hasSpend = metrics.includes('spend');
  return [
    ...
    hasSpend &&
      'A SECOND dataset "spend" is also provided: one row per region per spending CATEGORY, already summed over the whole period (so it has NO "date"). Fields: "region" (text), "category" (text), "spend" (AUD total), "no_txns" (transaction count), "cards_seen" (distinct cards). Use it for visitor-spend-by-category insight, and you may discuss spend per transaction (spend ÷ no_txns) in the narrative.',
    ...
    `"charts": an array of ${hasSpend ? '2–6' : '2–5'} objects, each { "metric": one of [${metricList}], "title": short string, "caption": one-line string, "chartSpec": a valid Vega-Lite v5 spec }.`,
    ...
    hasSpend &&
      'For the spend chart use "metric": "spend" and "data": {"name": "spend"}, and use ONLY the spend dataset\'s fields ("region", "category", "spend"). The dataset ALREADY contains just the top spending categories (summed over the period) — plot ALL of them and do NOT add any rank / top-N / filter transform of your own. Make it a HORIZONTAL bar: "category" on y (nominal) and "spend" on x (quantitative) — the category names are long (e.g. "PKG STORES/BEER/WINE/LIQUOR"), so they read far better on the y axis. Sort the y categories by total spend descending with "sort": { "op": "sum", "field": "spend", "order": "descending" }. NEVER put "date" on a spend chart. Encode region as "color" when multiple regions are present (the bars stack into each category total).',
